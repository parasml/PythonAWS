...
class Metadata:
    ...
    def destination_count(self, db_name, table_name):
        ...
        startTime = currentDateTime - timedelta(hours=2)
        startTime = startTime.replace(minute=58, second=0, microsecond=0)
        endTime = currentDateTime.replace(minute=5, second=0, microsecond=0)
        ...
        lsPrimaryKey = df['primarykey_value'].to_numpy()
        return lsPrimaryKey
    ...
    def source_count(self, table_name, created_datetime_column_name):
        ...
        query = session.query(sqlalchemy.func.count(getattr(source_table.columns, str_source_created_datetime_column))). \
            ...
            filter(getattr(source_table.columns, str_source_created_datetime_column).between(
                startTime, endTime))
        '''
        if table_name == 'bgorders':
            strPrimaryKey = 'bgorderid'
        else:
            strPrimaryKey = 'reqid'

        query = session.query(getattr(source_table.columns, strPrimaryKey)). \
            with_hint(source_table, "with (nolock)"). \
            filter(getattr(source_table.columns, str_source_created_datetime_column).between(
                startTime, endTime))
    ...
    # To check if the source a subset of destination ----------------
    def checkForSourceAsSubSet(self, lsPrimaryKey_Source, lsPrimaryKey_Destination):

        setSource = set(lsPrimaryKey_Source)
        setDestination = set(lsPrimaryKey_Destination)

        if setSource.issubset(setDestination): 
            return 0
        else:
            setDiff = setSource - setDestination
            return len(setDiff)
    ...
    def insert_metadata_grafana(self, strTable, created_datetime_column_name):
        ...
        # Count Match with Buffer -------------

        lsPrimaryKey_Source = self.source_count(table_name=strTable, created_datetime_column_name=created_datetime_column_name)
        lsPrimaryKey_Destination = self.destination_count(db_name='abshire', table_name=strTable)

        nDiff = self.checkForSourceAsSubSet(lsPrimaryKey_Source, lsPrimaryKey_Destination)
        print("nDiff = ", nDiff)


        df_metadata = pd.DataFrame({'dbname': 'abshire', 'tablename': strTable, 'source_count': len(lsPrimaryKey_Source),
                                    'destination_count': len(lsPrimaryKey_Source) + nDiff,
                                    'created_date_time': str(dt)}, index=[0])
        df_metadata["difference"] = nDiff

        # Count Match (Exact Date )----------
        '''
