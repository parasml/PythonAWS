...
GROUP_TABLES = ['abshire_wfdtable_lov']
#GROUP_TABLES = ['Abshire_BgOrders', 'Abshire_SearchReq']

#GROUP_TABLES = []   # For all tables in single json-------

def readData():
    pdData = pd.read_excel(FILE_PATH, sheet_name='Imp Tables', index_col=None, header=0)  
...
def getObjectLocator(strTableName, strSchemaName):
    ...
    data["schema-name"] = strSchemaName
...
# New Code
#&&&&&&&&&&&
# Need to Poulate GROUP_TABLES HERE ***************

#print("pdData = ", pdData.shape)
GROUP_TABLES = pdData['PublicationName'].unique()

#print("GROUP_TABLES = ", GROUP_TABLES)

print("hhhhhhhhhhhhhhhhhhhhhhhhhhhhh")

# Output CSV------------------------
pdOutput = pd.DataFrame(columns = ['PublicationName', 'TotalTabels', 'MappingJSON'])
print("pdOutput = ", pdOutput)
...
for group in GROUP_TABLES:
    ...
    json_Map = getCompleteMapping_JSON(df)

    #print("json_Map = ", json_Map)

    lsPublicationName.append(group)
    lsMappingJSON.append(json_Map)
    lsTotalTabels.append(df.shape[0])

    
pdOutput['PublicationName'] = lsPublicationName
pdOutput['MappingJSON'] = lsMappingJSON
pdOutput['TotalTabels'] = lsTotalTabels

print("pdOutput = ", pdOutput.shape)

pdOutput.to_csv('C:/Prashan/bitbucket/datalake/Kinesis-Stream/TableMapping/GroupJSON_Limited.csv', index=False)
