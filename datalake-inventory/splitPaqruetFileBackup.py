...
class splitPaqruetFile:
    ...
    def readFile(self):
        ...
        if self.objMetaData['record-create-time_column'] != '':
            strRecordCreateKey = config.MatchColumns(df.columns, self.objMetaData['record-create-time_column'])
        else:
    ...
    def removeDeleteRecords_DD_OLD(self, lsDeletRecords, table):
        ...
        for pKey in lsDeletRecords:
            ...
            try:
                response = table.query(
                    ...
                    FilterExpression=Key('primarykey_value').eq(int(pKey)) & Key('dms_timestamp').between(*dms_timestamp),
                    KeyConditionExpression=Key('dbname').eq(
                        self.objMetaData['dbname']) & Key('tablename').eq(self.objMetaData['tablename'])
                )

                #print("response = ", len(response['Items']))
                #print("response ID = ", response['Items'][0]['id'])
                ...
                print("Error in finding the record = ", pKey)

        print("Length arrRecord Id = ", len(arrRecordId))
        print("---QUERY TIME %s seconds ---" % (time.time() - start_time))
        # Delete Items now ------------------------------

        start_time1 = time.time()
        for strId in arrRecordId:
        ...
        #print("Enddd------------")
        print("---DELETE TIME %s seconds ---" % (time.time() - start_time1))
    '''
    ...
    def TestReconcileRecords_SQL(self, dfRaw):
        ...
        for index, row in dfTest.iterrows():
            #print(row)
            cursor.execute("INSERT INTO bgorders_reconcile([bgorderid],[dms_timestamp],[Op]) values(?,?,?)", row['BgOrderID'], row['dms_timestamp'], row['Op'])
